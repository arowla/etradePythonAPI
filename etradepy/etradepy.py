...
from etradepy import settings
# import urllib

"""
This is an attempt to implement a python compatible version of the etrade API
...
def urlRoot():
  """
  returns the root URL for the etrade API depending on sandbox_mode <boolean>
  """
...
def authorizeToken(resource_owner_key, resource_owner_secret):
  """
  ...
  with Browser() as browser:
    ...
    if browser.is_element_present_by_name('txtPassword', wait_time=0):
      ...
      browser.fill('USER', settings.username)
      time.sleep(3)
      ...
      browser.fill('PASSWORD', settings.userpass)
...
def accessToken(resource_owner_key, resource_owner_secret, verifier = None):
  """
...
def accessMethod(url, method = 'GET', payload = None):
  """
  This is a generic function that calls for url <str> and method <'GET' or
  'POST'>. If using 'POST', a payload <dict> should be supplied. The function
  returns a dict containing the output of the url called.
  """
  ...
  try:
    ...
    logging.warn('problem parsing response as json...', r)
    result = r.content
...
def listOrders(AcctNumber, marker = None):
  ...
  if numOfOrders == 0:
    ...
    logging.info('there are more orders')
...
def login():
    """
    Tries to initiate the login process and creates a token for the user
    """
    testState = listAccounts()

    if 'Error' in testState:
        logging.info('trying to renew token...')
    ...
    if 'Error' in testState:
        ...
        logging.info('trying to manually login... ')
        resource_owner_key, resource_owner_secret = getRequestToken()
        authCode = authorizeToken(resource_owner_key, resource_owner_secret)
        oauth_tokens = accessToken(resource_owner_key, resource_owner_secret, verifier = authCode)
